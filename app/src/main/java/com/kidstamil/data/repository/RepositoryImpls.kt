package com.kidstamil.data.repository
...
import com.kidstamil.data.mapper.DataMappers
...
import com.kidstamil.engine.PuzzleProfileEngine
import com.kidstamil.engine.KidsTamilEngine
import com.kidstamil.engine.KidsTamilGenerator
import com.kidstamil.engine.TutorialLevels
import com.kidstamil.util.DateUtils
...
class GameRepositoryImpl @Inject constructor(
    ...
    override suspend fun createNewGame(difficulty: Difficulty, levelNumber: Int): KidsTamilGame {
        ...
        val seed = KidsTamilGenerator.seedFromLevelNumber(levelNumber, difficulty)
        val level = KidsTamilGenerator.generate(seed, levelNumber, difficulty, generationProfile)
        return KidsTamilEngine.createInitialGame(level)
    }
    ...
    override suspend fun createTutorialGame(tutorialIndex: Int): KidsTamilGame? {
        val level = TutorialLevels.getTutorialLevel(tutorialIndex) ?: return null
        return KidsTamilEngine.createInitialGame(level)
    }

    override suspend fun createEndlessGame(wave: Int): KidsTamilGame {
        val seed = wave.toLong() * 7919L + System.currentTimeMillis() % 1000
        val level = KidsTamilGenerator.generate(seed, wave, Difficulty.ENDLESS)
        return KidsTamilEngine.createInitialGame(level)
    }

    override suspend fun saveGame(game: KidsTamilGame): Long {
    ...
    override suspend fun getGame(gameId: Long): KidsTamilGame? =
        gameDao.getById(gameId)?.let { DataMappers.fromEntity(it) }

    override suspend fun getInProgressGame(): KidsTamilGame? =
        gameDao.getInProgress()?.let { DataMappers.fromEntity(it) }

    override fun observeInProgressGame(): Flow<KidsTamilGame?> =
        gameDao.observeInProgress().map { it?.let { e -> DataMappers.fromEntity(e) } }

    override suspend fun completeGame(game: KidsTamilGame): KidsTamilGame {
    ...
    override suspend fun getLevel(seed: Long, levelNumber: Int, difficulty: Difficulty): KidsTamilLevel =
        KidsTamilGenerator.generate(seed, levelNumber, difficulty)
}
...
class ChallengeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getChallengeGame(record: ChallengeRecord): KidsTamilGame {
        val levelNumber = record.key.hashCode().and(0x7FFFFFFF) % 5000 + 1
        val level = KidsTamilGenerator.generateForChallenge(
...
class ProgressionRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateStatsAfterGame(game: KidsTamilGame) {
    ...
    private suspend fun updatePuzzleProfile(game: KidsTamilGame) {
    ...
    private fun updateFastest(current: Long, game: KidsTamilGame, difficulty: Difficulty): Long {
...
class PreferencesRepositoryImpl @Inject constructor(
    private val dataStore: com.kidstamil.data.local.PreferencesDataStore
) : com.kidstamil.domain.repository.PreferencesRepository {

    override fun getUserPreferences() = dataStore.preferencesFlow

    override suspend fun updatePreferences(
        transform: (com.kidstamil.domain.model.UserPreferences) -> com.kidstamil.domain.model.UserPreferences
