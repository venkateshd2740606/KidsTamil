...
object PuzzleProfileEngine {
    ...
    fun updateMetrics(current: PuzzleProfileMetrics, game: KidsTamilGame): PuzzleProfileMetrics {
        if (game.status != GameStatus.COMPLETED || game.level.isTutorial) return current

        val stepCount = max(game.level.stepCount, 1)
        val optimalScore = KidsTamilEngine.optimalScore(game)
        ...
        val isComplex = game.level.letterIndex >= 9
    ...
    fun adaptiveGenerationProfile(profile: PuzzleProfile): GenerationProfile {
        ...
        return GenerationProfile(letterOffsetModifier = modifier)
    }
